...
User data lives in a straightforward `users.json` file - easy to manage, easy to
understand.
...
This led to the introduction of an `assignedApps` array within the `users.json`
file for each user. Now, when requesting a token via /token/short or
/token/long, the client must also send an `Source` specifying which application
/ source / 'audience' the token is intended for.
...
**idently** checks if the requested source in the header is actually listed in
the user's assignedApps. If it's not a match, no token is issued. Furthermore,
the /token/verify endpoint now also requires the `Source` header to ensure the
token being verified was actually intended for that specific application. It
adds a simple but effective layer of authorization on top of the authentication.
...
limit.  
**idently** makes this easy. You can extend the User type in the TypeScript code
with your own optional properties. For example, you might add `specificRole` or
`budget`.
